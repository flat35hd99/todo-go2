...
export default function TodoList() {
    ...
    if (error || todos == undefined) return (
        <div>failed to load</div>
    )

    const handleUpdateTodo = (id: string, title: string) => {
        fetch("http://localhost:8080" + "/todo", {method: "PATCH", body: JSON.stringify({
            id,
            title
        })}).then(() => {
            mutate() // revoke SWR
...
function TodoCard({ todo, onUpdateTodo }: {todo: Todo, onUpdateTodo: (id: string, title: string) => void}) {
    ...
    return (
        <div className="border-solid border-2 my-2">
            ...
            {isEditting && (
                <>
                    <input value={inputtedTitle} onChange={(e) => setInputtedTitle(e.target.value)}></input>
                    <button onClick={() => {
                        setIsEditting(false)
